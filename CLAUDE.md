...
- Branch protection is deliberately not in use on this repo (decided 2026-08-15). The no-direct-push rule above stands as process discipline, not platform enforcement — treat a direct push to main as a mistake to flag and fix. Don't re-propose enabling platform protection; it's a settled call, not an oversight.
...
- Concretely, that's two modes, build/use both and pick per situation: **`--changed-since <commit>`** (day-to-day) diffs changed files against a saved baseline and walks the `require()`/import graph to find every affected function/module — a shared helper can affect dozens of downstream modules, not just the file that changed. **`--since-baseline [commit]`** (batch/catch-up) diffs straight against the baseline with no graph walk, for when many commits have accumulated (e.g. a full day of parallel-session merges) and a graph walk would leak through a shared core/hub file and balloon to nearly everything.
...
- Guard against a suspiciously-large diff being silently treated as normal — if the computed affected-set exceeds roughly 80% of everything, refuse and require an explicit override flag, mirroring the sanity check a human would do by hand before trusting an automated tool with it. This isn't theoretical: on 2026-08-16, `--changed-since` computed 485/487 functions (99%) affected after a day of accumulated merges, correctly refused, and pointed at `--since-baseline` instead — the refusal is the tool working as intended, not a bug to route around.
