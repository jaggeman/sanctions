...
describe('GET /api/search', () => {
  ...
  it('returns an empty array when every token is below the 2-char floor', async () => {
    const res = await request(api).get('/api/search').query({ q: 'a b' });
  ...
  it('queries array-contains on the first normalized token', async () => {
    snapshotDocs = [record()];
    const res = await request(api).get('/api/search').query({ q: 'Vladimir Putin' });
  ...
  it('applies the type filter as a second where() clause', async () => {
    snapshotDocs = [record()];
    await request(api).get('/api/search').query({ q: 'Vladimir', type: 'individual' });
  ...
  it('filters by source case-insensitively, in-memory', async () => {
    ...
    const res = await request(api).get('/api/search').query({ q: 'Vladimir', source: 'pep' });
    expect(res.body).toHaveLength(1);

    const res2 = await request(api).get('/api/search').query({ q: 'Vladimir', source: 'EU' });
    expect(res2.body).toHaveLength(0);
  });

  it('accepts a comma-separated source list', async () => {
    snapshotDocs = [record({ source: 'PEP' })];
    const res = await request(api).get('/api/search').query({ q: 'Vladimir', source: 'EU,PEP,UN' });
    expect(res.body).toHaveLength(1);
  });
  ...
  it('caps the requested limit at 100 regardless of what was asked for', async () => {
    ...
    const res = await request(api).get('/api/search').query({ q: 'Vladimir', limit: '99999' });
  ...
  it('falls back to the default limit of 20 when limit is not a number', async () => {
    ...
    const res = await request(api).get('/api/search').query({ q: 'Vladimir', limit: 'not-a-number' });
    expect(res.status).toBe(200);
...
describe('GET /api/sanctions/:id', () => {
  it('returns 404 when the document does not exist', async () => {
    docGetResult = { exists: false };
    const res = await request(api).get('/api/sanctions/DOES-NOT-EXIST');
  ...
  it('returns the record data when it exists', async () => {
    ...
    const res = await request(api).get('/api/sanctions/PEP-1');
    expect(res.status).toBe(200);
...
describe('POST /api/import', () => {
  it('rejects a non-array sources field', async () => {
    const res = await request(api).post('/api/import').send({ sources: 'EU' });
    expect(res.status).toBe(400);
  });
...
describe('POST /api/upload', () => {
  it('rejects a request with no file attached', async () => {
    const res = await request(api).post('/api/upload').field('source', 'PEP');
    expect(res.status).toBe(400);
