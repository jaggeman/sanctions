...
/**
 * Streams records for a source a chunk at a time (issues #5 and #31) rather
 * than needing every record for a source in one array — the real EU and
 * OFAC exports are too large to hold in memory at once. A session keeps only
 * what is genuinely small — the source's existing `{id -> status,
 * contentHash}` summary map and the set of ids seen so far — and writes
 * added/updated records as they arrive.
 *
