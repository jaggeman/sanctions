...
function mapEntityToRecord(entry: any): SanctionRecord | null {
  ...
  const toNameAlias = (c: AliasCandidate): NameAlias => ({
  ...
  // primaryNameOf/aliasNamesOf (src/shared/types.ts) trust array order, not
  // the `strong` flag, to decide which name is primary — so the entry
  // selectPrimary picked goes first, everything else keeps its original
  // relative order after it.
  let names: NameAlias[];
  if (candidates.length > 0) {
    const primaryIndex = selectPrimary(candidates);
    names = [
      toNameAlias(candidates[primaryIndex]),
      ...candidates.filter((_, i) => i !== primaryIndex).map(toNameAlias),
    ];
  } else {
