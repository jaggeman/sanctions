...
function deriveIdentificationStrings(identifications: Identification[]): string[] {
  const out: string[] = [];
  for (const id of identifications) {
    const entry = id.typeDescription ? `${id.typeDescription} ${id.number}` : id.number;
...
function mapDesignationToRecord(designation: any): SanctionRecord | null {
  ...
  if (candidates.length > 0) {
    ...
    primaryName = candidates[chosenIndex].wholeName;
    aliases = candidates.filter((_, i) => i !== chosenIndex).map((c) => c.wholeName);
  }

  const names: NameAlias[] = candidates.map((c) => ({ wholeName: c.wholeName, strong: c.isPrimary }));
  ...
  const passports = deriveIdentificationStrings(identifications);
